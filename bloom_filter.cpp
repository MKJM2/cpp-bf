...
BloomFilter::BloomFilter(size_t estimated_num_items, double false_positive_rate) {
    if (estimated_num_items == 0 || false_positive_rate <= 0.0 || false_positive_rate >= 1.0) {
        throw std::invalid_argument("Invalid parameters: n must be > 0, p must be between 0 and 1");
    }
...
BloomFilter::BloomFilter(size_t num_bits, size_t num_hashes)
    ...
    if (num_bits_ == 0 || num_hashes_ == 0) {
        throw std::invalid_argument("Invalid parameters: bits and hashes must be > 0");
    }
    initialize_bits();
}
...
void BloomFilter::calculate_optimal_params(size_t n, double p) {
    ...
    num_hashes_ = std::clamp<size_t>(num_hashes_, 1, 16); // Cap at 16 for practical efficiency
}
...
void BloomFilter::add(const char* data, size_t len) {
    const uint64_t hash1 = XXH64(data, len, SEED1);
    const uint64_t hash2 = XXH64(data, len, SEED2);
    
    for (size_t i = 0; i < num_hashes_; ++i) {
        // Fast alternative to modulo for bit index calculation
        const size_t bit_index = (hash1 + i * hash2) % num_bits_;
...
bool BloomFilter::might_contain(const char* data, size_t len) const {
    ...
    for (size_t i = 0; i < num_hashes_; ++i) {
        const size_t bit_index = (hash1 + i * hash2) % num_bits_;
        const size_t block_index = bit_index >> 6;      // Equivalent to / 64
        const size_t bit_offset = bit_index & 63;       // Equivalent to % 64
